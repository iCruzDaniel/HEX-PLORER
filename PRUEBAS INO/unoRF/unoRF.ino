...
const int vectorSize = 5;         // Tamaño del vector
int sendVector[vectorSize];       // Vector para enviar
int receivedVector[vectorSize];   // Vector para recibir

void setup() 
{
  Serial.begin(9600);
  radio.begin();
  radio.openWritingPipe(address[1]);  // Dirección para enviar datos
  radio.openReadingPipe(1, address[0]);  // Dirección para recibir datos
